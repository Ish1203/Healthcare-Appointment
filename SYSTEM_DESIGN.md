...
Every outbound email goes through `sendEmail()` in `emailService.js`, which **always writes to an `email_queue` table first** (status: pending → sent/failed) before attempting delivery via Nodemailer. If the SMTP call throws (bad credentials, provider downtime, rate limiting), the row is marked `failed` with an incremented `attempts` counter, but the **HTTP request to the user never fails because of it** — booking, cancellation, and visit-completion endpoints call `sendEmail()` without blocking the response, or wrap it in try/catch so email problems never break the core workflow. This is a graceful-degradation principle applied consistently across the system, mirroring how LLM failures are handled.
...
Both LLM calls (pre-visit and post-visit summaries) are powered by the **Groq API** using the `llama-3.3-70b-versatile` model via Groq's OpenAI-compatible `/v1/chat/completions` endpoint. The system prompt explicitly instructs the model to respond with ONLY valid JSON — no markdown fences, no explanation text — and the response is parsed defensively (`response.replace(/```json|```/g,'').trim()` then `JSON.parse`).

If the API call fails entirely (network error, invalid key, rate limit) or returns malformed JSON, the service catches the error and returns a hard-coded safe fallback object (e.g., urgency defaults to "Medium", a generic chief complaint, and three generic clarifying questions) instead of throwing. This guarantees the booking and visit-completion flow **always completes successfully** even if Groq is unavailable — the appointment is still booked, the doctor still gets a usable starting point, and the patient still receives a summary. Failed calls are flagged by `success: false` in the stored JSON for later auditing or reprocessing if needed.

## 6. Database Schema Rationale

SQLite (via `node-sqlite3-wasm`, a pure WebAssembly build with no native compilation required) was chosen for zero-setup local development and straightforward evaluation, while keeping the schema fully relational and normalized — separate `users`, `doctors`, `appointments`, `medications`, `leave_dates`, `email_queue`, and `notifications` tables with foreign key constraints. The synchronous, transactional nature of SQLite is what makes the atomic slot-hold check in section 1 reliable without needing external locking primitives. A swap to Postgres would require minimal query changes if horizontal scaling were ever needed, since the query layer is isolated in `db/database.js` and all route files use the same `db.prepare(sql).run/get/all()` interface.